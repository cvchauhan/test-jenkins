...
@Component({
  selector: 'app-job-assignment-internal',
  templateUrl: './job-assignment-internal.component.html',
  styleUrls: ['./job-assignment-internal.component.css'],
})
export class JobAssignmentInternalComponent implements OnInit {
  ...
  jobAssignStatus: any[] = JOB_ASSIGN_STATUS;
  ...
  assignJobD() {
    ...
    this.employeeData.forEach((emp, index) => {
      formData = {
        ...
        trainees: index == 0 ? this.trainees : [],
      };
    ...
    this.confirmationService.confirm({
      message:
        '<h1>Job Assignment for JobNo [' +
        this.jobStartAssignmentData['cJobNo'] +
        '] on [' +
        FullDate +
        ']  at [' +
        finalStartingAt +
      ...
      accept: () => {
        this.service.assignJob(tmpData).subscribe(
          (res) => {
            if (res['status'] == 200) {
    ...
    if (evt.checked) {
      this.jobScheduleTableData.forEach((element, ind) => {
        ...
        let formData = {
          PhoneNo: PhoneNo,
          EmpName: EmpName,
          CPatientNm: element['CPatientNm'],
          cJobNo: element['cJobNo'],
          nJobdid: element['nJobdid'],
          dStartDate: this.formatDate(this.Date),
          dStartTm: element['dStartTm'],
          dEndTm: element['dStartTm'],
          nEmpid: element['LastAssignedEmpid'],
          cJobResp: 'U',
          dRespDt: '',
          cReason: '',
          nAddrid: element['addressDetail'][0]['nAddId'],
          nUserid: localStorage.getItem('loginId'),
          transportation: null,
          nTransportPointId: null,
          trainees: [],
        };
      ...
      this.jobScheduleTableData.forEach((element, ind) => {
        ...
        this.multipleScheduleData = Utils.removeByAttr(
  ...
  rescheduleJob() {
    this.service.assignJob(this.multipleScheduleData).subscribe(
      (res) => {
        if (res['status'] == 200) {
          this.messageService.add({
            severity: 'success',
  ...
  getEmployeeBySrvSkills(srvSkillsId, scheduleDate, editObj) {
    ...
    this.service.getEmpBySrvSkill(srvSkillsId, scheduleDate1).subscribe(
      (res) => {
  ...
  getSelectedEmployeeData(editObj: any) {
    ...
    this.service.GetJobAssignData(editObj).subscribe(
      (res: any) => {
        this.tempTraineeData = [];
        this.employeeData = [];
        this.transportation = '';
        ...
        if (res && res.data.length) {
          ...
          for (let index = 0; index < Regulardata.length; index++) {
            ...
            let empIndex = this.empData.findIndex(
              (ind) => ind.nEmpId == Regulardata[index]['nEmpid']
            );
            if (empIndex >= 0) {
  ...
  getScheduleDate() {
    ...
    this.service
      .getAssignScheduleDataInternal(frmDate, branchID, this.jobStatus)
      .subscribe(
        (res) => {
  ...
  jobEditDialog(data, type) {
    ...
    this.getEmployeeBySrvSkills(
      this.jobStartAssignmentData['nSrvSkillid'],
  ...
  BranchLocation() {
    ...
    this.service.getBranch(empID).subscribe((res) => {
  ...
  history(data) {
    ...
    this.service.getJobHistoryData(jobId, startOfMonth, endOfMonth).subscribe(
      (res) => {
  ...
  searchTableF() {
    ...
    let roleName = '';
    this.service.getTablelList(roleName, empID).subscribe((res) => {
  ...
  getEmployees(SkillId) {
    let scheduleDate1 = this.formatDate(this.Date);
    this.service.getEmpBySrvSkill(SkillId, scheduleDate1).subscribe(
      (res) => {
        if (res) {
  ...
  remove(value, flag) {
    ...
    if (flag) {
      let removeIndex = this.traineeData.findIndex(
        (x) => x.nEmpid == value.nEmpid
      );
      if (removeIndex >= 0) {
      ...
      let removeIndex = this.tempTraineeData.findIndex(
        (x) => x.skillId == value.skillId && x.nEmpid == value.nEmpid
      );
      if (removeIndex >= 0) {
  ...
  onAddEmployee(value) {
    ...
    let obj = {
      nEmpid: this.EmpId,
      empName: EmpName,
      nTransportPointId: this.nTransportPointId,
      transportPointIdName: transportPointIdName,
    ...
    let empIndex = this.empData.findIndex((ind) => ind.nEmpId == this.EmpId);
    if (empIndex >= 0) {
      this.empData[empIndex]['disabled'] = true;
    }
    this.employeeData.push(obj);
  ...
  onStaffChange(value) {
    ...
    this.service.getEmployeeData(empID, startOfMonth, endOfMonth).subscribe(
      (res) => {
        ...
        this.datesArr.forEach((elem, ind) => {
          if (elem.JobExecution.length) {
            let tmpObj = {
              title:
                elem['JobExecution'][0]['cJobno'] +
                ' , ' +
                elem['JobExecution'][0]['PatientName'] +
                ' , ' +
                elem['JobExecution'][0]['dStartTm'] +
                ' , ' +
                elem['JobExecution'][0]['dEndtm'] +
                ' , ' +
                elem['JobExecution'][0]['CustName'],
