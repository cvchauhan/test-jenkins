...
@Component({
  selector: 'app-stock-deposit',
  templateUrl: './stock-deposit.component.html',
  styleUrls: ['./stock-deposit.component.scss']
})
export class StockDepositComponent implements OnInit {
   ...
   manageSaveButtonEnable(){
    ...
    for(let i =0;i<this.empTableQty.length;i++){
      if(this.empTableQty[i].nQty >0){
        if(this.empTableQty[i].nQty > this.empTableQty[i].customerInHand){
          this.saveButtonEnabled = false;
          break;
        }else{
  ...
  let branch = this.issueForm.get('issueBranch').value;
    this._manageStockService.loadJobDataDeposit(q,this.issueBranchId).subscribe((res) => {
...
getIssueJobno(jobId){
  ...
  this.empTableQty = [];
  this._manageStockService.getJobN(jobId).subscribe(
    res=>{
  ...
  showOTPDialog(){
    ...
    formData['cTrType'] = existingFormValues['cTrType']? existingFormValues['cTrType'] : "D";
    formData['dStkDate'] = this.formatDate(existingFormValues['dStkDate']);
    formData['nEmpid'] = existingFormValues['nEmpid'] ;
    formData['nAcid'] = existingFormValues['nJobid']['nAcid'];
    formData['nUserid'] = localStorage.getItem('loginId');
